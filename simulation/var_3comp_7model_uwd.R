...
#                                   format = "df")               
#     m_params <- apply(params, MARGIN = 2, FUN = mean)
#     ordn_mus <- m_params[1:5]
#     ordn_sigmas <- m_params[6:10]
#     wts <- m_params[11:15] 
#     wts[wts < 0] <- 0
...
#                                   format = "df")               
#     m_params <- apply(params, MARGIN = 2, FUN = mean)
#     ord_mus <- m_params[1:5]
#     ord_sigmas <- m_params[6:10]
#     wts <- m_params[11:15] 
#     wts[wts < 0] <- 0
#     ord_wts <- wts/sum(wts)
#     
...
#                                   format = "df")               
#     m_params <- apply(params, MARGIN = 2, FUN = mean)
#     ind_mus <- m_params[1:5]
#     ind_sigmas <- m_params[6:10]
#     wts <- m_params[11:15] 
#     wts[wts < 0] <- 0
#     ind_wts <- wts/sum(wts)
#     
...
#     qind <- function(p) {q(ind_mix)(p)} 
    ...
    uwd1s <- c(uwd1_cltn, uwd1_clt)
    uwd2s <- c(uwd2_cltn, uwd2_clt)

    uwd1os <- c(uwd1_cltno, uwd1_clto) 
    
    kss <- c(ks_cltn, ks_clt)
    
    klds <- c(cltn_kl, clt_kl)
    
    tvs <- c(cltn_tv, clt_tv)
...
write.csv(distance, paste0("sim_scores/", dist, "3_", sample_type, "/", "size", n, "_probs", length(levels[[p]]), "_scores.csv"), row.names = FALSE)
