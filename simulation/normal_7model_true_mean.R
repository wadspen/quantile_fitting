...
distance <- foreach(replicate = 1:reps,
                    ...
                    ,.combine = rbind) %dopar% {
                      ...
                      params_cltn <- fit_cltn[[2]]$draws(variables = c("mu", "sigma"), 
                                                    format = "df") %>% 
                                      as.data.frame()
                      params_ordn <- fit_ordn[[2]]$draws(variables = c("mu", "sigma"), 
                                                    format = "df") %>% 
                        as.data.frame()
                      params_clt <- fit_clt[[2]]$draws(variables = c("mu", "sigma"), 
                                                   format = "df") %>% 
                        as.data.frame()
                      params_ord <- fit_ord[[2]]$draws(variables = c("mu", "sigma"), 
                                                   format = "df") %>% 
                        as.data.frame()
                      params_ind <- fit_ind[[2]]$draws(variables = c("mu", "sigma"), 
                                                   format = "df") %>% 
                        as.data.frame()
                      ...
                      for (d in 1:length(ds)) {
                        ...
                        cltn_mu  <- params_cltn[ds[d], 1]
                        cltn_sd  <- params_cltn[ds[d], 2]
                        
                        ordn_mu  <- params_ordn[ds[d], 1]
                        ordn_sd  <- params_ordn[ds[d], 2]
                        
                        clt_mu   <- params_clt[ds[d], 1]
                        clt_sd   <- params_clt[ds[d], 2]
                        
                        ord_mu   <- params_ord[ds[d], 1]
                        ord_sd   <- params_ord[ds[d], 2]
                        
                        ind_mu   <- params_ind[ds[d], 1]
                        ind_sd   <- params_ind[ds[d], 2]
                        ...
                        cltnx <- dnorm(kls, cltn_mu, cltn_sd)
                        ordnx <- dnorm(kls, ordn_mu, ordn_sd)
                        cltx  <- dnorm(kls, clt_mu,  clt_sd)
                        ordx  <- dnorm(kls, ord_mu,  ord_sd)
                        indx  <- dnorm(kls, ind_mu,  ind_sd)
                        ...
                        cltn_tv[d] <- dens_dist(
                          function(x) dnorm(x, cltn_mu, cltn_sd),
                          ddist
                        )
                        ordn_tv[d] <- dens_dist(
                          function(x) dnorm(x, ordn_mu, ordn_sd),
                          ddist
                        )
                        clt_tv[d] <- dens_dist(
                          function(x) dnorm(x, clt_mu, clt_sd),
                          ddist
                        )
                        ord_tv[d] <- dens_dist(
                          function(x) dnorm(x, ord_mu, ord_sd),
                          ddist
                        )
                        ind_tv[d] <- dens_dist(
                          function(x) dnorm(x, ind_mu, ind_sd),
                          ddist
                        )
