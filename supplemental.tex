...
Figure \ref{fig:norm_quant_sens} shows distance 
results of QM for normal quantiles for increasing number of quantiles $K$. 
For the QGP, ORD, and IND models, there is a tendency for the fits to 
improve as $K$ increases. However, the fits may worsen slightly when 
extreme quantiles are included as is seen between $K = 11$ and $K = 15$. The
worsening behavior is tempered for larger $n$.
...
modeled 95\% credible interval over the 500 replications. Coverage is faceted 
...
\begin{figure}[hbt!]
\centering
  \includegraphics[width = 1\linewidth]{Images/exp_quant_sensitive.png}
\caption{Distance between the true exponential distribution 
and the estimated QM predictive distribution averaged over the 500 replications
for increasing number of quantiles $K$. 
Distances
include the UWD1, TV, and KLD, and sample size $n$ is an additional facet.}
...
to La and MIX distributions as described in section 4.2.
Figures \ref{fig:mixC_K_la} and \ref{fig:mixC_K_la} show simulation study 
for La and MIX distributions similar to the results shown for
the EV distribution in section 4.2.
...
\begin{figure}[hbt!]
\centering
\includegraphics[width=1.1\linewidth]{Images/uwd1_dpm_vs_fm_la.png}
\caption{UWD1 distance between Laplace distribution $La(0,1)$
...
\begin{figure}[hbt!]
\centering
\includegraphics[width=1.1\linewidth]{Images/uwd1_dpm_vs_fm_mix.png}
\caption{UWD1 distance between 2 component normal mixture distribution
$\text{MIX}$
and QM estimated distributions for increasing sample size $n$. Plots are 
faceted by number of quantiles $K$ (columns) 
and number of mixture distribution 
...
normal components $C$
to use in the distribution assigned for use in fitting the QGP under the FM
framework.
Figure \ref{fig:mix_comps} shows the UWD1 distance between 
the true distributions and the QM fits 
simulated averaged over 200 simulated
replicates. 
% Table \ref{tab:mix_comps} shows the average values. 
The results 
show that increasing $C$ up to around 8 components improves the QM fits
for these simulated data, but as $C$ grows larger, the fits worsen. 
The tendency to worsen decreases as the number of quantiles $K$ increases. 
Thus,
we would recommend setting $C$ to between 4 and 8 components.
...
\begin{figure}[hbt!]
\centering
  \includegraphics[width=.6\textwidth]{Images/ncomp_uwd1.png}
\caption{UWD1 distance averaged over 200 simulation replicates
for QGP under the FM construction 
fits of quantile simulated from extreme value, Laplace, and
mixture distributions and for 
number of quantiles $K \in \{11, 15, 23\}$. Sample size is $n = 500$}
\label{fig:mix_comps}
\end{figure}


Figure \ref{fig:est_mix_tdpm} shows boxplots for the number of mixture 
distribution components accounting for 99\% of the posterior predictive 
distribution for QM of unknown distribution families under the 
TDPM construction where the number of mixture components is $C = 20$. 
The data come from 
200 simulation replicates from the EV, La, and MIX distributions and 
number of quantiles $K \in \{11, 15, 23\}$. The TDPM prior regularizes the 
mixture distributions to generally have 6 or fewer significant components. 



\begin{figure}[hbt!]
\centering
  \includegraphics[width=.6\textwidth]{Images/est_mix_tdpm.png}
\caption{
Number of mixture distribution components after regularization under the TDPM
construction where $C = 20$ for QM fits of quantiles from 200 simulated 
replicates from extreme value, Laplace, and
mixture distributions and for 
number of quantiles $K \in \{11, 15, 23\}$. Sample size is $n = 500$}
\label{fig:est_mix_tdpm}
\end{figure}
