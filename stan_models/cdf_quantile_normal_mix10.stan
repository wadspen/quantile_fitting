...
functions{
  ...
  vector GMInv_CDF(vector Q, data real p, vector pi, vector mu, 
    ...
    cdf = pi[1]*exp(normal_lcdf(Q | mu[1], sigma[1])) +
          ...
          + pi[9]*exp(normal_lcdf(Q | mu[9], sigma[9]))
          + pi[10]*exp(normal_lcdf(Q | mu[10], sigma[10]))
          ;
    z[1] = cdf - p;
    return z;
  }
  ...
  real GM_CDF(real y, vector mu, vector sigma, vector pi) {
    ...
    prob = pi[1]*normal_cdf(y | mu[1], sigma[1])
           ...
           + pi[9]*normal_cdf(y | mu[9], sigma[9])
           + pi[10]*normal_cdf(y | mu[10], sigma[10])
           ;
  ...
  real GM_PDF(vector mu, vector sigma, vector pi, real y) {
    ...
    density = pi[1]*exp(normal_lpdf(y | mu[1], sigma[1])) +
              ...
              + pi[7]*exp(normal_lpdf(y | mu[7], sigma[7]))
              + pi[8]*exp(normal_lpdf(y | mu[8], sigma[8]))
              + pi[9]*exp(normal_lpdf(y | mu[9], sigma[9]))
              + pi[10]*exp(normal_lpdf(y | mu[10], sigma[10]))
              + .000001
              ;
