...
parameters {
  ...
  real<lower=0> sigma;
  vector<lower=0>[n_components] lambda;
  real<lower=0> tau;
  vector<lower=0>[n_components] rho;
  
}

transformed parameters {
  vector<lower=0>[n_components] sigma_rho = (.2 * tau^2 * lambda^2) ./ (1 + .2 * tau^2 * lambda^2);
  vector<lower=0>[n_components] gamma;
  for (i in 1:n_components) gamma[i] = sum(rho[1:i])/sum(rho);
  simplex[n_components] pi = gamma/sum(gamma);
